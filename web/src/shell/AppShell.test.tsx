...
import { AppShell, SESSION_SNAPSHOT_POLL_MS } from "./AppShell";
...
// The header's session-warning banner reads `activeSession.warnings` off the
// per-session snapshot. The condition it describes is recorded server-side
// WHILE the session runs — later than the snapshot the UI first cached, and
// with no SSE channel of its own — so both halves matter: AppShell must render
// what the snapshot carries, and it must keep re-asking for the snapshot.
describe("AppShell — session warning banner", () => {
  function sessionWith(warnings?: { code: string; harness?: string; reason?: string }[]) {
    return {
      id: "conv_warned",
      agentId: "ag_x",
      agentName: "coder",
  ...
  it("shows no banner while the cached snapshot carries no warning", () => {
    mockConversations([]);
    useSessionMock.mockReturnValue({
      session: sessionWith() as never,
      isLoading: false,
      error: null,
    });
  ...
  // A real refetch, not a remount: the whole point of the poll is that the
  // banner appears in a session the user never reloaded, so the test drives
  // the real hook + a real QueryClient and lets the interval fire. A remount
  // would pass even if the poll were removed.
  it("renders the banner once the poll refetches a snapshot carrying the warning", async () => {
    const { useSession: realUseSession } =
      await vi.importActual<typeof UseSessionModule>("@/hooks/useSession");
    useSessionMock.mockImplementation(realUseSession);
    mockConversations([]);

    let warned = false;
    const fetchMock = vi.fn((input: RequestInfo | URL) => {
      const url = input.toString();
      if (url.includes("/v1/sessions/conv_warned")) {
    ...
    try {
      ...
      const snapshotFetches = () =>
        fetchMock.mock.calls.filter((call) =>
          call[0].toString().includes("/v1/sessions/conv_warned"),
        );
      expect(snapshotFetches()).toHaveLength(1);
      // Only the cache-cold read pops the runner's caches (see useSession).
      expect(snapshotFetches()[0]![0].toString()).toContain("refresh_state=true");

      // The server records the warning while the session runs; the next poll
      // tick is the only thing that can surface it.
      warned = true;
      await act(async () => {
        await vi.advanceTimersByTimeAsync(SESSION_SNAPSHOT_POLL_MS + 1);
      });
      expect(screen.getByTestId("session-warning-banner")).toBeInTheDocument();
      expect(
        screen.getByTestId("session-warning-subagent_routing_unenforced").textContent,
      ).toContain("codex-native");
      // A second request really happened, and it left the runner caches alone.
      expect(snapshotFetches()).toHaveLength(2);
      expect(snapshotFetches()[1]![0].toString()).not.toContain("refresh_state=true");
    } finally {
      vi.useRealTimers();
      vi.unstubAllGlobals();
    }
  ...
  it("ignores a warning code the UI has no copy for", () => {
    mockConversations([]);
    useSessionMock.mockReturnValue({
      session: sessionWith([{ code: "some_future_code", reason: "x" }]) as never,
      isLoading: false,
      error: null,
    });
    renderShell("/c/conv_warned");
  ...
  it("polls the open session's snapshot so a later warning can arrive at all", () => {
    // Without an interval the query is staleTime:Infinity and chatStore only
    // refetches on bind — the banner would need a hard reload to ever appear.
    mockConversations([]);
    useSessionMock.mockReturnValue({
      session: sessionWith() as never,
      isLoading: false,
    ...
    renderShell("/c/conv_warned");
    expect(useSessionMock).toHaveBeenCalledWith("conv_warned", {
