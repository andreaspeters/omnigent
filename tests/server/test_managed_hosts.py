...
def test_parse_valid_mesos_compose_config_builds_parameterized_factory(
    ...
    cfg = parse_sandbox_config(
        {
            ...
            "mesos": {
                "image": "ghcr.io/omnigent-ai/omnigent-host:v0.8.2",
                "env": ["OPENAI_BASE_URL"],
                "compose_url": "https://mesoscompose.weave.local:10002/",
                "username": "user",
                "verify_ssl": False,
    ...
    assert cfg.launcher_factory() is fake
    assert fake.image == "ghcr.io/omnigent-ai/omnigent-host:v0.8.2"
    assert fake.env == ["OPENAI_BASE_URL"]
    assert fake.compose_url == "https://mesoscompose.weave.local:10002/"
    assert fake.username == "user"
    assert fake.verify_ssl is False
    assert fake.target_hostname == "andreas-ki.lab.internal"
